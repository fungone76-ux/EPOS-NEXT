...
TASK_PROFILES: dict[LLMTask, LLMTaskProfile] = {
    ...
    LLMTask.AUDIT_NARRATION: LLMTaskProfile(
        ...
        system_instruction=(
            ...
            "rhetorical questions, challenges, refusals, opinions, normative statements, or "
            "uncertainty as violations when they are consistent with that NPC's authorized "
            "reaction. Statements such as 'Non possiamo permetterci ambiguità' or rhetorical "
            "questions such as 'Perché mai dovresti fidarti di me?' are interpersonal dialogue "
            "and should produce no finding when supported by reaction tone or intent. If an "
            "NPCDialogueDraft invents a concrete factual assertion about the NPC's past conduct, "
            "personal history, achievements, promises, events, other people, or the world, classify "
            "it as unsupported_npc_fact unless separately grounded by authorized evidence. Example: "
            "'Ho sempre agito con trasparenza' is unsupported_npc_fact without evidence of that "
            "history. Reserve unsupported_world_claim for WorldNarrationDraft or for dialogue that "
            "asserts an external world fact as objective truth. Report only concrete contradictions "
            "or genuinely invented factual claims, not harmless dialogue rhetoric or characterization."
        ),
    ),
